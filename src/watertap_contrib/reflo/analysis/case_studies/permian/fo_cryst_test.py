...
def set_operating_conditions(m, operating_condition, **kwargs):
    ...
    # m.fs.treatment.feed.properties[0].dens_mass = rho
...
def build_cryst(m, cryst_config):
    ...
    print(total_feed_H2O_mass)
    print(total_feed_NaCl_mass)
    ...
    results = solver.solve(treat.mec)
...
if __name__ == "__main__":
    permian_fo_config = {
    "feed_vol_flow": 0.15, # initial value for fo model setup
    ...
    # m.fs.energy.cst.unit.costing.costing_package.trough_surrogate.cost_per_storage_capital = 31
...
#%% Parametric
    ...
    recovery_ratios = [0.349,0.35,0.351, 0.361,0.369, 0.389, 0.391, 0.401, 0.433, 0.44, 0.45,0.461, 0.469,0.475,0.486, 0.494, 0.511,0.52,0.525,0.531,0.539,0.541,0.545]
    # recovery_ratios = [0.2 + i*0.02 for i in range(18)]
    ...
    df.to_csv('csv_results/FO_Cryst_RPT.csv')
#%% plotting
    import pandas as pd
    from watertap_contrib.reflo.analysis.case_studies.permian import *

    results_file = f"csv_results/FO_cryst_RPT.csv"
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame.from_dict(results_dict)
    df.to_csv('csv_results/FO_Cryst_grid_frac.csv')

#%% Plotting grid frac
    import pandas as pd
    from watertap_contrib.reflo.analysis.case_studies.permian import *

    results_file = f"csv_results/FO_Cryst_grid_frac.csv"
    df = pd.read_csv(results_file)
    ...
    ax_dict = dict(xlabel="Grid Fraction (Heat)", ylabel="LCOW (\$/m$^3$)")
...
if __name__ == "__main__":
    ...
    m = run_permian_FO_cryst(operating_condition,
                            ...
                            CST_config,)
    results_dict = build_results_dict(m, skips=["diffus_phase_comp"])
    heat_cost = np.linspace(0.00447, 0.011175, 10)
    results_dict["heat_cost"] =[]
    ...
    for v in heat_cost:
        try:
            ...
            heat_price =value(pyunits.convert( v  * pyunits.USD_2023, to_units=pyunits.USD_2018))
            ...
            results_dict = results_dict_append(m, results_dict)
            LCOW.append((v, value(m.fs.treatment.costing.LCOW)))
            results_dict["heat_cost"].append(v)
        except:
            LCOW.append((v,'fail'))

    df = pd.DataFrame.from_dict(results_dict)
    df.to_csv('csv_results/FO_Cryst_heat_price.csv')

#%% Plotting heat price
    import pandas as pd
    from watertap_contrib.reflo.analysis.case_studies.permian import *

    results_file = f"csv_results/FO_Cryst_heat_price.csv"
    df = pd.read_csv(results_file)
    ...
    ax_dict = dict(xlabel="Heat Price ($/kWh)", ylabel="LCOW (\$/m$^3$)")
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame.from_dict(results_dict)
    df.to_csv('csv_results/FO_Cryst_RPT_naclprice.csv')
# %% Plot nacl price
    import pandas as pd
    from watertap_contrib.reflo.analysis.case_studies.permian import *

    results_file = f"csv_results/FO_Cryst_RPT_naclprice.csv"
    df = pd.read_csv(results_file)
    ...
    ax_dict = dict(xlabel="NaCl Price ($/kg)", ylabel="LCOW (\$/m$^3$)")
...
if __name__ == "__main__":
    ...
    results_dict["cost_per_total_aperture_area"] =[]
    ...
    for v in cst_price:
        try:
            ...
            m.fs.energy.cst.unit.costing.costing_package.trough_surrogate.cost_per_total_aperture_area = v
            results = solver.solve(m)
            assert_optimal_termination(results)
            results_dict = results_dict_append(m, results_dict)
            LCOW.append((v, value(m.fs.costing.LCOW)))
            results_dict["cost_per_total_aperture_area"].append(v)
        except:
            LCOW.append((v,'fail'))

    df = pd.DataFrame.from_dict(results_dict)
    df.to_csv('csv_results/FO_Cryst_cost_per_total_aperture_area.csv')
...
if __name__ == "__main__":
    ...
    results_dict["cost_per_storage_capital"] =[]
    ...
    for v in storage_price:
        try:
            ...
            m.fs.energy.cst.unit.costing.costing_package.trough_surrogate.cost_per_storage_capital = v
            results = solver.solve(m)
            assert_optimal_termination(results)
            results_dict = results_dict_append(m, results_dict)
            LCOW.append((v, value(m.fs.costing.LCOW)))
            results_dict["cost_per_storage_capital"].append(v)
        except:
            LCOW.append((v,'fail'))

    df = pd.DataFrame.from_dict(results_dict)
    df.to_csv('csv_results/FO_Cryst_cost_per_storage_capital.csv')
